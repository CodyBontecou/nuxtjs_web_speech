  ...
  export default {
    ...
    methods: {
      ...
      beginSpeechRecognition() {
        ...
        const initial = this.words[this.iteration].english;
        that.$store.commit('setInitial', initial)
        speak(initial, recognition);


        recognition.onresult = function (event) {
          const last = event.results.length - 1;
          const answer = event.results[last][0].transcript.toLowerCase();

          that.$store.commit('setAnswer', answer)

          setTimeout(that.beep, 500);
      ...
      beep() {
        let sound;
        if (this.words[this.iteration].spanish === this.answer) {
