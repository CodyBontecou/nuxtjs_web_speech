    ...
    <div v-if="compatibleBrowser">
      ...
      <div class="flex pt-20 m-10 text-center justify-around">
        <div class="w-1/3 p-5 bg-indigo-500 rounded-lg shadow-lg">
          <p id="initial_word" data-cy="initial_word" class="text-white uppercase tracking-wider font-semibold"></p>
        </div>
        <div class="w-1/3 p-5 bg-indigo-500 rounded-lg shadow-lg">
          <p id="answer" data-cy="answer" class="text-white uppercase tracking-wider font-semibold"></p>
        </div>
      </div>
...
<script>
  ...
  export default {
    computed: {
      ...mapGetters(['compatibleBrowser', 'words', 'word', 'iteration'])
    },
    mounted() {
    ...
    methods: {
      checkBrowserCompatibility() {
        if (!window.webkitSpeechRecognition && !window.speechRecognition) {
          this.$store.commit('setCompatibleBrowser', false)
        }
      },
      beginSpeechRecognition() {
        ...
        const answer = this.words[this.iteration].english;
        ...
        recognition.onresult = function (event) {
          ...
          that.$store.commit('setWord', word)

          diagnostic.textContent = word;

          setTimeout(beep, 500);
          setTimeout(that.beginSpeechRecognition, 1000);
          function beep() {
            let sound;
            if (that.words[that.iteration].spanish === that.word) {
      ...
      handleIteration() {
        if (this.iteration >= this.words.length - 1) {
          this.$store.commit('resetIteration')
        } else {
          this.$store.commit('increaseIteration')
        }
      }
    }
  }
